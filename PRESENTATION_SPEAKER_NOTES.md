...
English: In healthcare, safety comes first. We use a rule-first mechanism so red flags such as chest pain with breathing difficulty, blood in stool, or fast-spreading rash with fever trigger priority warnings. The result page shows reasoning, disclaimer, and human review.

中文：医疗场景中安全优先。我们使用规则优先机制，让胸痛加呼吸困难、便血、皮疹快速扩散伴发热等红旗症状优先触发风险提示。同时结果页显示判断依据、免责声明，并保留人工复核。

## Slide 10. Benefits / 量化收益

English: These numbers are simulated course evaluation assumptions, not real hospital data. They show quantifiable thinking: intake time decreases from 8 minutes to 3 minutes, structured completeness improves from 60% to 90%, and daily capacity increases from 50 to 90 cases.

中文：这些数值是课程模拟评估假设，不是真实医院数据。我们用它们展示量化思维：平均采集时间从 8 分钟降到 3 分钟，结构化完整率从 60% 提升到 90%，日处理能力从 50 个提升到 90 个。
...
中文：目标客户包括诊所、门诊部门、互联网医疗平台、企业健康管理和社区医疗中心。商业模式可以是 B2B SaaS 订阅，也可以通过 API 接入平台。它比固定问卷更灵活，比纯大模型更安全可控。

## Slide 12. Reflection and Q&A / 反思与答疑

English: The current project uses simulated cases, has limited symptom coverage, and has not been clinically validated. Future work includes expanding specialty rules, adding database and access control, involving clinical experts, and running compliant pilot studies. Our conclusion is that the realistic value of AI in healthcare is not replacing doctors, but improving intake efficiency, risk visibility, and human decision support.

中文：当前项目使用模拟病例，覆盖症状有限，也没有真实临床验证。未来需要扩展专科规则库、加入数据库和权限控制、邀请临床专家审核，并在合规前提下进行小规模试点。我们的结论是：AI 在医疗中最现实的价值不是替代医生，而是提升预问诊效率、风险可见性和人工决策支持。
...
中文：医学诊断风险高、责任边界复杂。预问诊和导诊支持更安全，也更适合课程原型。
